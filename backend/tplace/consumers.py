...
def get_redis_client():
    ...
    if _redis_client is None:
        _redis_client = redis.from_url(settings.TPLACE_REDIS_URL, decode_responses=True)

    return _redis_client


class TplaceConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        if text_data:
            ...
            await redis_client.ltrim(
                self.history_key, -settings.TPLACE_HISTORY_LIMIT, -1
            )
            await redis_client.expire(self.history_key, settings.TPLACE_HISTORY_TTL)
