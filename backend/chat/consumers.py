...
def get_redis_client():
    ...
    if _redis_client is None:
        _redis_client = redis.from_url(settings.CHAT_REDIS_URL, decode_responses=True)
...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        await redis_client.ltrim(self.history_key, -settings.CHAT_HISTORY_LIMIT, -1)
    ...
    async def send_history(self):
        ...
        raw_messages = await redis_client.lrange(
            self.history_key, -settings.CHAT_HISTORY_LIMIT, -1
        )
        messages = []
